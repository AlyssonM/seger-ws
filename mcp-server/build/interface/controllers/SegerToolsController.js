...
export class SegerToolsController {
    ...
    registerDadosConsolidadosTool() {
        this.server.tool("dados-consolidados", "Obtém dados consolidados de faturas de uma instalação incluindo custos detalhados (energia, demanda, iluminação pública, bandeiras, impostos). Use esta ferramenta para responder perguntas sobre custos específicos nas faturas.", {
    ...
    registerObterDadosFaturasTool() {
        this.server.tool("obter_dados_faturas", "Obtém dados detalhados das faturas incluindo TODOS os custos: energia, demanda, iluminação pública, bandeiras tarifárias, impostos PIS/COFINS/ICMS. Ideal para responder perguntas sobre custos específicos de qualquer componente da fatura.", {
            codInstalacao: z.string()
            ...
            try {
                ...
                const iluminacaoPublica = dados.dados?.map((fatura) => {
                    const ilum = fatura.dados?.componentes_extras?.find((comp) => comp.descricao?.toLowerCase().includes('ilum'));
                    return {
                ...
                }).filter((item) => item.iluminacao_publica);
                return {
                    content: [
                        {
                            type: "text",
                            text: `📊 **Dados Detalhados das Faturas da Instalação ${codInstalacao}**\n` +
                                ...
                                (iluminacaoPublica?.length > 0 ?
                                    iluminacaoPublica.map((item) => `- ${item.mes}: ${item.iluminacao_publica.descricao} = R$ ${item.iluminacao_publica.valor.toFixed(2)}`).join('\n') :
